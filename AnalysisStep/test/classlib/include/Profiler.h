...
public:
    ...
    TH1F* FillProfile(TString input_file_name, float lumi, TH1F* hist, const std::function<bool(Tree*)>& cut, const std::function<float(Tree*)>& var, bool normalize, float start_fraction = 0.0, float end_fraction = 1.0, bool fast_reweighting = false);
