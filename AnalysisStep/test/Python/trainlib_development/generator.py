...
#        self.as_matrix = as_matrix
    ...
    def preprocessed_generator(self):
        for H1_data, H0_data in self.raw_generator():
            ...
            # H1_data["__target__"] = 1.0
            # H0_data["__target__"] = 0.0            

            # data_chunk = pd.concat([H1_data, H0_data])
            
            # # scramble it
            # input_data = data_chunk.sample(frac = 1)

            # target_data = {"target": input_data["__target__"]}

            # have the preprocessor run on all the data combined
