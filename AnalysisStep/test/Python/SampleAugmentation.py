...
def augment_file(data_inpath, data_outpath, data_file, mcolls):
    ...
    fcoll = FileCollection([data_outfile], 0.0, 1.0)
    ...
    for mcoll in mcolls:
        out_branches.append(np.array(mcoll.predict(indata)))
        prepared_dtype.append((mcoll.name, 'f4'))

    # make it into the correct type and shape
    new_branches = np.array(out_branches, dtype = prepared_dtype)
    new_branches = np.swapaxes(new_branches, 0, 1)
