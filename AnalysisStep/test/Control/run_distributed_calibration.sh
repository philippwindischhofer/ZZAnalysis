...
do
    ...
    echo $BIN_DIR$CALIBRATOR $AUGMENTATION_TRAINING_DIR $CALIBRATION_TRAINING_DIR "0.0" "1.0" "&>" $CALIBRATION_TRAINING_LOGFILE >> $CALIBRATION_TRAINING_SCRIPT

    echo "#!/bin/bash" > $CALIBRATION_VALIDATION_SCRIPT
    echo $BIN_DIR$CALIBRATOR $AUGMENTATION_VALIDATION_DIR $CALIBRATION_VALIDATION_DIR "0.0" "1.0" "&>" $CALIBRATION_VALIDATION_LOGFILE >> $CALIBRATION_VALIDATION_SCRIPT

    echo "#!/bin/bash" > $CALIBRATION_TEST_SCRIPT
    echo $BIN_DIR$CALIBRATOR $AUGMENTATION_TEST_DIR $CALIBRATION_TEST_DIR "0.0" "1.0" "&>" $CALIBRATION_TEST_LOGFILE >> $CALIBRATION_TEST_SCRIPT
done
