...
do
    ...
    BENCHMARK_DIR=$CAMPAIGN_DIR$RUN"benchmark_priors/"
    COMP_DIR=$CAMPAIGN_DIR$RUN"comp_priors/"
    BENCHMARK_SETTINGS_DIR=$CAMPAIGN_DIR$RUN"settings_benchmark_priors/"
    ...
    BENCHMARK_SCRIPT=$BENCHMARK_SETTINGS_DIR"run_benchmark_priors.sh"
    BENCHMARK_LOGFILE=$BENCHMARK_SETTINGS_DIR"log_benchmark_priors.txt"

    echo "#!/bin/bash" > $BENCHMARK_SCRIPT

    # launch the benchmarking
    echo $BIN_DIR$BENCHMARKER $CALIBRATION_DIR $AUGMENTATION_DIR $BENCHMARK_DIR $PRIOR_DIR "&>" $BENCHMARK_LOGFILE >> $BENCHMARK_SCRIPT
...
JOBS=`find * | grep run_benchmark_priors.sh$`
